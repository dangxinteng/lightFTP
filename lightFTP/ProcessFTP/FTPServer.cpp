...
bool FTPServer::ShareMemory()
{
    FILE *pStream;
    key_t kShareMem;
    int iShareMem;

    pStream = fopen("./m_bRun.share", "wb");//wb rb a(append)
    if (pStream)
        fclose(pStream);
    kShareMem = ftok("./m_bRun.share", 'a');
    if (kShareMem == -1)
    {
        perror("ftok fail.");
        return false;
    }
    iShareMem = shmget(kShareMem, sizeof(bool), 0666 | IPC_CREAT);
    if (iShareMem<0)
    {
        perror("shmget fail.");
        return false;
    }
    m_bRun= (bool *)shmat(iShareMem, 0, 0);
